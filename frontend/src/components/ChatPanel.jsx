...
const ChatPanel = () => {
	...
	useEffect(() => {
		if (!authUser) return;
		authFetch(`${API_BASE_URL}/api/chat/friends`).then(r => r.json()).then(d => setFriends(d.friends || []));
		authFetch(`${API_BASE_URL}/api/chat/pending-requests`).then(r => r.json()).then(d => setPendingRequests(d.friendRequests || []));
	}, [authUser]);

	useEffect(() => {
		if (!socket) return;
		const handle = (msg) => {
			if (activeChat !== msg.senderId) setUnreadFrom(prev => new Set([...prev, msg.senderId]));
		};
	...
	const acceptRequest = async (username) => {
		const res = await authFetch(`${API_BASE_URL}/api/chat/friend-request/${username}/accept`, { method: "POST" });
	...
	const declineRequest = async (username) => {
		await authFetch(`${API_BASE_URL}/api/chat/friend-request/${username}/decline`, { method: "POST" });
		setPendingRequests(prev => prev.filter(r => r.username !== username));
	};
	...
	return (
		<>
			<button onClick={() => setOpen(!open)} className="fixed bottom-6 right-6 w-14 h-14 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg flex items-center justify-center z-40 transition-all">
				{open ? <IoClose size={24} /> : <FaCommentDots size={22} />}
				{!open && totalNotif > 0 && (
					<span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center font-bold">{totalNotif}</span>
				)}
			</button>
			...
			{open && (
				<div className="fixed bottom-24 right-6 w-72 bg-gray-900 border border-gray-700 rounded-xl shadow-2xl z-40 overflow-hidden">
					<div className="px-4 py-3 bg-gray-800 border-b border-gray-700">
						<h3 className="font-semibold text-white flex items-center gap-2"><MdMarkChatUnread size={18} className="text-blue-400" />Messages</h3>
					</div>
					{pendingRequests.length > 0 && (
						<div className="border-b border-gray-700">
							<p className="text-xs text-yellow-400 font-semibold px-4 py-2 bg-yellow-500/10">{pendingRequests.length} Friend Request{pendingRequests.length > 1 ? "s" : ""}</p>
					...
					<div className="max-h-64 overflow-y-auto">
						{friends.length === 0 && pendingRequests.length === 0 && (
							<p className="text-gray-500 text-xs text-center py-8 px-4">No friends yet. Search for a user and send a friend request to start chatting!</p>
						)}
						{friends.map(friend => {
							const isOnline = onlineUsers.includes(friend.username);
							const hasUnread = unreadFrom.has(friend.username);
							return (
								<button key={friend.username} onClick={() => { setActiveChat(friend.username); setUnreadFrom(prev => { const n = new Set(prev); n.delete(friend.username); return n; }); setOpen(false); }} className="flex items-center gap-3 w-full px-4 py-3 hover:bg-gray-800 transition-colors">
			...
			{activeChat && <ChatWindow username={activeChat} onClose={() => setActiveChat(null)} />}
		</>
	);
};
